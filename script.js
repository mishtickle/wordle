...
for (let i = 0; i < 6; i++) {
    ...
    row.addEventListener("input", async (event) => {
        ...
        if (rowFilled) {
            ...
            if (!valid) {
                const letterContainers = rows[activeRow].querySelectorAll('.letter-container')
                letterContainers.forEach(c => c.classList.add('invalid'))
                setTimeout(() => letterContainers.forEach(c => c.classList.remove('invalid')), 700)
                return
            }
    ...
    row.addEventListener("keydown", (event) => {
        ...
        if (event.key === "Backspace" && input.value === "") {
            const previousLetterContainer = input.parentElement.previousElementSibling
